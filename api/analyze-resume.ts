...
// FIX: Cast multer middleware to 'any' to resolve Express handler overload issue.
app.post('/api/analyze-resume', upload.single('resume') as any, async (req: any, res: any) => {
    if (!req.file) {
    ...
    return res.status(503).json({ 
        success: false, 
        message: 'AI Assistant is temporarily disabled. Feature coming soon.' 
    });
});
